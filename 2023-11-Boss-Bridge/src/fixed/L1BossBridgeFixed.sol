...
contract L1BossBridgeFixed is Ownable, Pausable, ReentrancyGuard {
    ...
    mapping(address account => uint256 deposit_amount) public depositAmountOf;
    uint256 public totalDepositAmount;
    mapping(address signer => uint256 nonce) public getSignerNonce;
    ...
    function setSigner(address account, bool enabled, uint256 lastNonce) external onlyOwner {
        signers[account] = enabled;

        if (enabled) {
            getSignerNonce[account] = lastNonce;
        } else {
            delete getSignerNonce[account];
        }
    }
    ...
    function depositTokensToL2(address l2Recipient, uint256 amount) external whenNotPaused {
        ...
        if (totalDepositAmount + amount > DEPOSIT_LIMIT) {
            revert L1BossBridge__DepositLimitReached();
        }
        token.safeTransferFrom(msg.sender, address(vault), amount);

        totalDepositAmount += amount;
        depositAmountOf[msg.sender] += amount;
        ...
        whenNotPaused
    {
         ...
         */
        require(depositAmountOf[msg.sender] >= amount, "Insufficient deposit amount");

        bytes memory call_data = abi.encodeCall(IERC20.transferFrom, (address(vault), to, amount));
        // target value chainId nonce call_data
        bytes memory data = abi.encode(address(token), 0, chainId, nonce, call_data);
        sendToL1(v, r, s, data);
    }
     ...
     */
    function sendToL1(uint8 v, bytes32 r, bytes32 s, bytes memory data) private nonReentrant whenNotPaused {
        /**
         * @audit:public修饰符使得该函数可被任意调用
         * 此处若signer作恶，因此signer可以构造任意的message，执行任意的函数
         */
        address signer = ECDSA.recover(MessageHashUtils.toDataWithIntendedValidatorHash(address(this), data), v, r, s);

        if (!signers[signer]) {
            revert L1BossBridge__Unauthorized();
        }

        (address target, uint256 value, uint256 chainId, uint256 nonce, bytes memory call_data) =
            abi.decode(data, (address, uint256, uint256, uint256, bytes));

        // 防止重放
        require(chainId == block.chainid, "Invalid chainId");
        require(nonce == getSignerNonce[signer], "Invalid nonce");

        getSignerNonce[signer] = nonce + 1;

        (bool success,) = target.call{ value: value }(call_data);
        if (!success) {
