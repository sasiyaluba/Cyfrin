...
contract ThunderLoanFixed is Initializable, OwnableUpgradeable, UUPSUpgradeable, OracleUpgradeable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        uint256 amountReceived = endingBalance - startingBalance;

        // @audit: 在铸造代币时，直接使用用户传入的amount值而非计算转入的token数量
        uint256 mintAmount = (amountReceived * assetToken.EXCHANGE_RATE_PRECISION()) / exchangeRate;
    ...
    function setAllowedToken(IERC20 token, bool allowed) external onlyOwner returns (AssetToken) {
        // 确保输入的IERC20都是标准的ERC20
        bytes memory decimals_calldata = abi.encodeWithSignature("decimals()");
        (bool success, bytes memory data) = address(token).staticcall(decimals_calldata);
        ...
        if (allowed) {
            ...
            AssetToken assetToken = s_tokenToAssetToken[token];
            // @audit: 在此处将token停止进行flashloan之后，原本的用户无法使用redeem再提款
            require(token.balanceOf(address(assetToken)) == 0, "Token still has balance");
    ...
    function getCalculatedFee(IERC20 token, uint256 amount) public view returns (uint256 fee) {
        ...
        uint256 numerator = (amount * getPriceInWeth(address(token))) * s_flashLoanFee;
        uint256 deneominator = s_feePrecision * s_feePrecision;

        fee = numerator / deneominator;
